...
Client.prototype.putDocument = function (key, doc, options, callback) {
  ...
  if (metadata) {
    ...
    if (metadata['Raven-Entity-Name']) {
      headers['Raven-Entity-Name'] = metadata['Raven-Entity-Name'];
    }
  }
...
/**
 * Deletes documents by index
 */
Client.prototype.deleteDocuments = function(indexName, options, callback) {
  ...
  self.httpClient.delete('bulk_docs/' + indexName, queryOptions, { allowStale: true}, {}, function (error, result) {
    ...
    if (!error && result.statusCode === 200 && options.waitForNonStaleResults) {
      ...
      if (data.IsStale) {
        ...
        return setTimeout(function () {
          self.deleteDocuments(indexName, options, callback);
        }, 500);
...
Client.prototype.deleteDocument = function(key, options, callback) {
  var self = this,
      root = 'docs/';
  ...
  this.httpClient.delete(root + key, null, null, {}, function (error, result) {
    // HTTP response 201 is considered a success
    return callback && callback(error, { ok: result.statusCode === 201, response: result});
  });  
...
Client.prototype.patchDocument = function(key, doc, callback) {
  var self = this,
      root = 'docs/';
  
  return self.httpClient.patch(root + key, null, doc, null, function(error, result) {
    return callback && callback(error, { ok: result.statusCode === 201, response: result});
  });
}
...
Client.prototype.createIndex = function(indexName, doc, options, callback) {
  var self = this, 
      root = 'indexes/';
  ...
  function convertToString(stringOrArray) {
    if (typeof stringOrArray === 'object' && stringOrArray.length) {
      return stringOrArray.join('\r\n');
    } else return stringOrArray;
  };

  if (doc.Map)
    doc.Map = convertToString(doc.Map);

  if (doc.Reduce)
    doc.Reduce = convertToString(doc.Reduce);

  // If our document has metadata - extract it into the request headers and remove it from the document before serialization.

  this.httpClient.put(root + indexName, null, doc, {}, function (error, result) {
    // HTTP response 201 is considered a success
    return callback && callback(error, { ok: result.statusCode === 201, response: result});
  });  
};
